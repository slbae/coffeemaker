...
public class APIRecipeController extends APIController {
    ...
    /**
     * REST API method to allow editing a Recipe that is currently stored in the
     * CoffeMaker's Repository of Recipes. This is done by using the
     * updateRecipe method, than using a API Post function.
     *
     * @param editedRecipe
     *            The new edited Recipe
     * @return Success if the recipe could be edited; an error if the recipe
     *         does not exist
     */
    @PutMapping ( BASE_PATH + "/recipes/{name}" )
    public ResponseEntity editRecipe ( final Recipe editedRecipe ) {

        // Pull the original recipe from the recipeService
        final Recipe orginalRecipe = service.findByName( editedRecipe.getName() );

        // Check that the recipe actually exists
        if ( null == orginalRecipe ) {
            return new ResponseEntity( errorResponse( "No recipe found for name " + editedRecipe.getName() ),
                    HttpStatus.NOT_FOUND );
        }

        // Use the updateRecipe method to modify the Recipe
        orginalRecipe.updateRecipe( editedRecipe );

        // Save the newly updated Recipe
        service.save( orginalRecipe );

        // Return a successful message
        return new ResponseEntity( successResponse( editedRecipe.getName() + " was edited successfully" ),
                HttpStatus.OK );
     ...
     * to update an existing Recipe by automatically converting the JSON
    ...
    public ResponseEntity updateRecipe ( @PathVariable final String name, @RequestBody final Recipe recipe ) {
        ...
        if ( oldRecipe == null ) {
            return new ResponseEntity( errorResponse( "No recipe found with name " + recipe.getName() ),
                    HttpStatus.NOT_FOUND );
        }
